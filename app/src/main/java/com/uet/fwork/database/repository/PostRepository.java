...
public class PostRepository extends Repository {
    ...
    public void insert(
            DatabaseReference reference, PostModel post,
            @Nullable OnQuerySuccessListener<Boolean> listener) {
        String postId = rootDatabaseReference.push().getKey();
        post.setPostId(postId);
        reference.setValue(post)
                .addOnFailureListener(exception -> {
                    Log.d(LOG_TAG, "Insert post failed " + post);
                ...
                .addOnSuccessListener(unused -> {
                    Log.d(LOG_TAG, "Insert post successful " + post);
                    if (listener != null) {
                        listener.onSuccess(true);
                    }
                })
                .addOnCanceledListener(() ->
                        Log.d(LOG_TAG, "Insert post cancelled " + post));
    }
